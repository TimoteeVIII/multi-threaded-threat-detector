...
// create the packet handler for pcap_loop
void my_packet_handler(u_char *args, const struct pcap_pkthdr *header, const u_char *packet);
void my_packet_handler(u_char *args, const struct pcap_pkthdr *packet_header, const u_char *packet_body)
{
    // send packet details to dispatch
    dispatch(packet_header, packet_body, v);
    if(v){ // dump packet data if -v flag set to 1
      dump(packet_body, packet_header->len);
    }
    return;
}
...
void sniff(char *interface, int verbose) {
  ...
  pcap_loop(pcap_handle, -1, my_packet_handler, NULL);
}
